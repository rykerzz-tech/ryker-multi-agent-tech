...
Ryker MultiAgent Tech has first-class support for [Cursor IDE](https://cursor.sh/). The CLI generates a `.cursor/` directory containing rules, slash commands, and MCP configuration — natively understood by Cursor.
...
npx ryker-multi-agent-tech init --cursor-only

# Generate both .windsurf/ and .cursor/ (multi-IDE projects)
npx ryker-multi-agent-tech init --cursor

# Re-generate after .windsurf/ updates
npx ryker-multi-agent-tech init --cursor-only --force
```
...
3. **After updating `.windsurf/`:** Re-run `npx ryker-multi-agent-tech init --cursor-only --force` to sync
...
Pass `--force` to overwrite: `npx ryker-multi-agent-tech init --cursor-only --force`
...
- Regenerate: `npx ryker-multi-agent-tech init --cursor-only --force`
