...
All notable changes to **ryker-multi-agent-tech-dashboard** will be documented in this file.
...
- **Dependency sync** — Bumped `ryker-multi-agent-tech` dependency version to v2.7.2 (no API key required for init)
...
- **[C2]** Docker deployment broken — `NEXT_PUBLIC_*` env vars are build-time in Next.js; added `ARG` + `ENV` in Dockerfile, `build.args` in docker-compose, and `RYKER_API_URL` for rewrites in `next.config.js`
- **[C3]** SSRF vulnerability on `/api/metrics` — server-side fetch now uses `RYKER_API_URL` (not client-exposed) with URL allowlist validation (`localhost`, `127.0.0.1`, `::1`, `0.0.0.0`)
...
- **[H8]** SSRF allowlist blocks Docker network hostnames — `METRICS_PROXY_ALLOWED_HOSTS` env var allows custom hostnames (e.g. `ryker-api`)
...
- **[M5]** `next.config.js` rewrites hardcoded to `localhost:3000` — now uses `RYKER_API_URL` env var
...
- `Dockerfile` — added `ARG RYKER_API_URL`, `ARG RYKER_WS_URL` for build-time env injection; port changed from 3000 to 3001
- `docker-compose.yml` — `ryker-dashboard` now uses `build.args` for env vars
- `next.config.js` — rewrites destination derived from `RYKER_API_URL` env var; added `NEXT_PUBLIC_APP_VERSION`; removed `/ws` rewrite
...
**Infrastructure**
- Dockerfile + docker-compose integration (ryker-dashboard on :3001)
