    ...
    const state = await agentRuntime.runAgent({
      ...
      onStep: (step, state) => {
        ...
        if (showFullTrace) {
          ...
          if (step.toolCalls.length > 0 && !jsonMode) {
            step.toolCalls.forEach(tc => {
              if (tc.error) {
                console.log(chalk.red(`  ✗ Tool: ${tc.tool} (FAIL)`));
              } else {
                console.log(chalk.cyan(`  ✔ Tool: ${tc.tool} (PASS)`));
              }
            });
          }
        }

        if (!jsonMode) spinner = ora("กำลังทำงาน...").start();
      },
    });
