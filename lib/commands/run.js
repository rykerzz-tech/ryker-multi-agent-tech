...
async function run(input, options = {}) {
  ...
  const showFullTrace = trace || verbose;
  ...
  if (!jsonMode) {
    console.log(chalk.cyan("1. 🔍 วิเคราะห์คำขอ: สำเร็จ"));
    console.log(chalk.cyan("2. 📅 วางแผนงาน: สำเร็จ"));
    console.log(chalk.cyan("3. 🎯 เลือก Agent: สำเร็จ"));
    console.log("\n" + EliteNexus.getActiveAgentDisplay(agentName, input, showFullTrace) + "\n");
    console.log(chalk.cyan("4. ⚡ ดำเนินการ..."));
  }

  let spinner = jsonMode ? null : ora("กำลังทำงาน...").start();
  let stepCount = 0;
  ...
  try {
    const state = await agentRuntime.runAgent({
      ...
      onStep: (step, state) => {
        ...
        if (showFullTrace) {
          ...
          if (step.toolCalls.length > 0) {
            step.toolCalls.forEach(tc => {
              if (tc.error) {
                ...
                console.log(chalk.cyan(`  ✔ Tool Execution (${tc.tool}) สำเร็จ`));
          ...
          // Compact / Default mode: do not output tool calls internal details
        }

        if (!jsonMode) spinner = ora(`กำลังทำงาน (Step ${stepCount})...`).start();
      },
    });
    ...
    if (!jsonMode) {
      console.log(chalk.cyan("5. 🧪 ตรวจสอบผลลัพธ์: สำเร็จ"));
      console.log(chalk.cyan("6. 📋 สรุปงาน:\n"));
    }
    ...
    if (state.status === "completed") {
      ...
      console.log(chalk.yellow("\n⚠ สิ้นสุดเนื่องจากถึงขีดจำกัดสูงสุด (Max Steps reached). ผลลัพธ์บางส่วน:\n"));
      console.log(state.output);
    }

    // Structured Reporting System (Normal vs Debug Mode)
    console.log("\n" + EliteNexus.generateReport(state, trace ? "TRACE" : (verbose ? "VERBOSE" : "NORMAL")) + "\n");
