...
async function run(input, options = {}) {
  ...
  if (!jsonMode) {
    console.log(chalk.cyan(`\n🚀 [Elite-Nexus Engine] กำลังเริ่มประมวลผล Agent: ${agentName}`));
    console.log(chalk.gray(`   Input Task: ${input.slice(0, 80)}${input.length > 80 ? "..." : ""}`));
    if (provider) console.log(chalk.gray(`   Target Provider: ${provider}`));
    console.log("");
  }
