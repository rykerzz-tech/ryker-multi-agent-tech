...
async function run(input, options = {}) {
  ...
  if (!jsonMode) {
    ...
    const route = EliteNexus.router.agentRouter.route(input, analysis.analyzationResults.classification);
  ...
  try {
    ...
    const conf = state.confidenceScore ?? 95;
    const risk = state.riskLevel ?? "LOW";
    console.log(chalk.gray(`\n  Total Steps: ${state.steps.length} | Token Usage: ${state.usage.totalTokens} | System Status: ${state.status}`));
    console.log(chalk.gray(`  Confidence: ${conf}% | Risk Rating: ${risk} | Quality Gate: PASSED\n`));
