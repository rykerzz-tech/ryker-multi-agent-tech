...
async function run(projectDir, options = {}) {
  ...
  if (config.configExists(projectDir)) {
    console.log(chalk.yellow(".agent/ or .windsurf/ already exists. Run `ryker-multi-agent-tech update` instead.\n"));
    return;
  }
  ...
  if (options.interactive) {
    console.log(chalk.cyan("\n🚀 Ryker MultiAgent Tech — Smart Init\n"));
    ...
    console.log(chalk.cyan(`\n🚀 Ryker MultiAgent Tech — Quick Init`));
  ...
  try {
    ...
    console.log(`\n  ${chalk.gray("Next:")} ryker-multi-agent-tech add skill <name>`);
    console.log(`        ${chalk.gray("ryker-multi-agent-tech test")}\n`);
  } catch (err) {
...
async function runRooOnly(projectDir, options) {
  ...
  try {
    ...
    const answers = { agentName: path.basename(projectDir) || "ryker-agent" };
...
## Instructions
Follow the Ryker Agent Kit rules from GEMINI.md.
...
function generateWindsurfrules(projectDir, answers) {
  ...
  const content = `# Ryker MultiAgent Tech — Auto-read by Windsurf IDE
...
- \`ryker-multi-agent-tech run "<input>"\` — Execute agent
- \`ryker-multi-agent-tech chat\` — Interactive session
`;
