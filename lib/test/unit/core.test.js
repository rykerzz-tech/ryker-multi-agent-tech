...
async function runAll() {
  ...
  await test("sandboxExec allows safe command", () => {
    const cmd = process.platform === "win32" ? "git" : "ls";
    const args = process.platform === "win32" ? ["--version"] : [];
    const result = guardrails.sandboxExec(cmd, args, { timeout: 5000, cwd: ROOT });
  ...
  await test("shell.exec blocks command substitution $(...)", async () => {
    const result = await toolRegistry.BUILTIN_TOOLS["shell.exec"]({ command: 'git "--version$(whoami)"' });
    assert.ok(result.error);
    assert.ok(result.error.includes("forbidden pattern"));
  });

  await test("shell.exec blocks backtick command substitution", async () => {
    const result = await toolRegistry.BUILTIN_TOOLS["shell.exec"]({ command: "git `whoami`" });
    assert.ok(result.error);
  });
  ...
  await test("shell.exec allows safe command", async () => {
    const cmd = process.platform === "win32" ? "git --version" : "echo hello";
    const expected = process.platform === "win32" ? "git version" : "hello";
    const result = await toolRegistry.BUILTIN_TOOLS["shell.exec"]({ command: cmd });
    assert.ok(result.stdout.includes(expected));
    assert.strictEqual(result.exitCode, 0);
  });

  await test("shell.exec handles quoted args", async () => {
    const cmd = process.platform === "win32" ? 'git log "-n 1"' : 'echo "hello world"';
    const result = await toolRegistry.BUILTIN_TOOLS["shell.exec"]({ command: cmd });
    assert.ok(result.exitCode === 0 || result.error);
  });

  await test("shell.exec allows > inside quoted string (false positive fix)", async () => {
    const cmd = process.platform === "win32" ? 'node -v' : 'echo "2 > 1"';
    const result = await toolRegistry.BUILTIN_TOOLS["shell.exec"]({ command: cmd });
    assert.strictEqual(result.exitCode, 0);
  });
  ...
  await test("parseNoteFile extracts frontmatter and body", () => {
    ...
    const tmp = path.join(os.tmpdir(), `ryker-note-${Date.now()}.md`);
  ...
  await test("parseNoteFile handles missing frontmatter", () => {
    ...
    const os = require("os");
    const { parseNoteFile } = require("../../commands/run-from-file");
    const tmp = path.join(os.tmpdir(), `ryker-note-noheader-${Date.now()}.md`);
  ...
  await test("parseNoteFile handles empty frontmatter", () => {
    ...
    const os = require("os");
    const { parseNoteFile } = require("../../commands/run-from-file");
    const tmp = path.join(os.tmpdir(), `ryker-note-empty-fm-${Date.now()}.md`);
    fs.writeFileSync(tmp, "---\n---\n\nBody only\n");
    try {
      const result = parseNoteFile(tmp);
      assert.deepStrictEqual(result.frontmatter, {});
  ...
  await test("parseNoteFile handles UTF-8 BOM prefix", () => {
    ...
    const os = require("os");
    const { parseNoteFile } = require("../../commands/run-from-file");
    const tmp = path.join(os.tmpdir(), `ryker-note-bom-${Date.now()}.md`);
    fs.writeFileSync(tmp, "\uFEFF---\nagent: frontend\n---\n\nDesign a button\n");
    try {
      const result = parseNoteFile(tmp);
