...
async function runAll() {
  ...
  await test("EliteNexusOS high-level 6 stages flow executes cleanly", () => {
    const input = "build backend web api with tests";
    const understandResult = EliteNexus.understand(input);
    assert.ok(understandResult.intent);
    assert.ok(understandResult.complexity);

    const planResult = EliteNexus.plan(input, understandResult);
    assert.ok(planResult.plan);
    assert.ok(planResult.schedule);

    const agent = EliteNexus.createAgentInstance("TestSpecialist");
    const execResult = EliteNexus.execute(planResult.plan, agent);
    assert.strictEqual(execResult.status, "executing");

    const verifyResult = EliteNexus.verify("Output code draft", input);
    assert.ok(verifyResult.status);
    assert.ok(verifyResult.confidence);

    const deliverResult = EliteNexus.deliver(verifyResult, "Output code draft");
    assert.strictEqual(deliverResult.completed, true);

    const recoverResult = EliteNexus.recover({ status: "FAIL" }, 0);
    assert.strictEqual(recoverResult.shouldRetry, true);
  });
