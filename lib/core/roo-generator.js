 ...
 * Converts ryker agent definitions → Roo custom modes
 */
...
function buildModes(agentsDir) {
  ...
  // Convert ryker agents → Roo modes
  ...
  for (const file of files) {
    ...
    const roleDefinition = [
      ...
      "Follow the Ryker Agent Kit rules from GEMINI.md. Apply clean-code principles.",
    ].filter(Boolean).join(" ");
...
function generateRoorules(projectDir, answers = {}) {
  ...
  const agentName = answers.agentName || "ryker-agent";
  const provider = answers.provider || "mock";
  const memory = answers.memory || "none";

  const content = `# Ryker MultiAgent Tech — Auto-read by Roo Code
...
- \`ryker-multi-agent-tech update\` — Update to latest agent library
...
You are Roo with Ryker Agent Kit active.
