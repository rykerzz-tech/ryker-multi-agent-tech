...
async function runAgent(options) {
  ...
  // Elite-Nexus Orchestration & Pre-execution Analysis
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (toolCalls.length === 0) {
      const { BLACKLIST } = require("./quality-gate");
      const reviewerResult = EliteNexus.reflection.reviewer.review(assistantContent, BLACKLIST);
      const reflectionResult = EliteNexus.reflection.reflector.reflect(assistantContent, safeInput);
      const confidenceResult = EliteNexus.reflection.confidence.calculate(assistantContent);
      const riskResult = EliteNexus.reflection.risk.analyze(assistantContent);
      const critiqueResult = EliteNexus.reflection.critique.critique(assistantContent);

      const passesReview = reviewerResult.approved && riskResult.riskLevel !== "HIGH" && reflectionResult.completenessScore >= 0.8;
      ...
      if (!passesReview && (state.retryCount || 0) < 3) {
        ...
        const issues = [
          ...
          ...(critiqueResult.observations || [])
        ];
        if (issues.length === 0) issues.push("Draft completeness score below threshold");
...
โปรดปรับปรุงคำตอบใหม่และแก้ไขจุดบกพร่องตามรายการข้างต้น โดยห้ามแสดงประเด็นเหล่านี้ออกมาอีก`;
        ...
        stepRecord.error = `Quality reflection retry #${state.retryCount}`;
