...
async function runAgent(options) {
  ...
  // Build system prompt
  const rawSystemPrompt = buildSystemPrompt(agentSpec, skillInstructions, projectProfile, outputFormat, "", provider);
...
โปรดปรับปรุงคำตอบใหม่และแก้ไขจุดบกพร่องตามรายการข้างต้น โดยห้ามแสดงประเด็นเหล่านี้ออกมาอีก`;
  ...
  // Autonomous Self-Reflection Evaluation
  if (state.output && typeof state.output === "string") {
    state.reflection = EliteNexus.cognitive.reflectionEngine.evaluateOutput(state.output);
  }
