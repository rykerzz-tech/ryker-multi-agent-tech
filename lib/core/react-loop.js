...
async function runAgent(options) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    if (toolCalls.length === 0) {
      const checkContext = {
        changedFiles: state.steps.map(s => s.action?.args?.TargetFile || s.action?.args?.path || s.action?.args?.target).filter(Boolean),
        ...
        runtimeChecked: state.steps.some(s => s.toolCalls?.some(tc => tc.tool === "run_command" && (tc.args.CommandLine?.includes("start") || tc.args.CommandLine?.includes("node")))),
        runtimePassed: state.steps.some(s => s.toolCalls?.some(tc => tc.tool === "run_command" && (tc.args.CommandLine?.includes("start") || tc.args.CommandLine?.includes("node")) && !tc.error)),
