    ...
    function doFetch(url, redirectCount) {
      return new Promise((resolve) => {
        ...
        const reqOpts = {
          ...
          method,
          headers: {
            "User-Agent": "ryker-multi-agent-tech/fetch",
            ...headers,
          },
...
BUILTIN_TOOLS["code.execute"] = async (args) => {
  const vm = require("vm");
...
BUILTIN_TOOLS["git.status"] = async (args) => {
  const projectRoot = args.projectRoot || args.cwd || process.cwd();
  try {
    const { execFileSync } = require("child_process");
    const gitCmd = process.platform === "win32" ? "git.exe" : "git";
    const stdout = execFileSync(gitCmd, ["status", "--short"], {
      cwd: projectRoot,
      encoding: "utf-8",
      timeout: 10000,
    });
    const branch = execFileSync(gitCmd, ["branch", "--show-current"], {
      cwd: projectRoot,
      encoding: "utf-8",
      timeout: 5000,
    }).trim();
    return { branch, status: stdout.trim(), clean: stdout.trim().length === 0 };
  } catch (err) {
    return { error: `git status error: ${err.message}` };
  }
};

// Aliases
LEGACY_ALIAS["file_read"] = "fs.read";
LEGACY_ALIAS["file_write"] = "fs.write";
LEGACY_ALIAS["web_search"] = "web.search";
LEGACY_ALIAS["code_execute"] = "code.execute";
LEGACY_ALIAS["git_status"] = "git.status";

TOOL_SCHEMAS["code.execute"] = { required: ["code"], optional: ["timeout"], capabilities: ["code_execution", "sandbox"] };
TOOL_SCHEMAS["git.status"] = { required: [], optional: ["cwd"], capabilities: ["git", "version_control"] };

function discoverPlugins(projectDir = process.cwd()) {
  const toolsDir = path.join(projectDir, ".nexus", "tools");
  ...
  try {
    ...
    for (const file of files) {
      if (file.endsWith(".js")) {
        try {
          const modPath = path.join(toolsDir, file);
          delete require.cache[require.resolve(modPath)];
          const mod = require(modPath);
          if (mod && mod.name && typeof mod.execute === "function") {
            registerTool(mod.name, mod.execute, mod.schema || {});
            if (mod.capabilities) {
              TOOL_SCHEMAS[mod.name] = { ...TOOL_SCHEMAS[mod.name], capabilities: mod.capabilities };
            }
          }
