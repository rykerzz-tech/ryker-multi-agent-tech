...
const FAILOVER_CHAIN = ["openai", "claude", "gemini", "deepseek", "mistral", "groq", "local", "mock"];
...
function buildFailoverChain(preferredProvider) {
  ...
  const filtered = chain.filter(p => {
    ...
    if ((p === "gemini" || p === "google") && !(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || process.env.GOOGLE_GENERATIVE_AI_KEY)) return false;
    if (p === "deepseek" && !process.env.DEEPSEEK_API_KEY) return false;
    if (p === "mistral" && !process.env.MISTRAL_API_KEY) return false;
...
function resolveProvider() {
  ...
  // Antigravity IDE injects GEMINI_API_KEY, GOOGLE_API_KEY, or GOOGLE_GENERATIVE_AI_KEY
  if (process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY || process.env.GOOGLE_GENERATIVE_AI_KEY) {
    return "gemini";
  }
...
async function callLLMStreamWithFailover(messages, llmOpts, onChunk = () => {}) {
  const preferredProvider = llmOpts.provider || resolveProvider();
  const chain = buildFailoverChain(preferredProvider);
  let lastError;
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    const breakerName = ensureLlmBreaker(provider);
    if (!circuitBreaker.canExecute(breakerName)) {
      logger.warn(`Circuit breaker OPEN for ${provider} — skipping in stream`);
      continue;
    }

    try {
      const response = await llmProviders.callLLMStream(messages, { ...llmOpts, provider }, onChunk);
      circuitBreaker.recordSuccess(breakerName);
      if (provider === "local") markOllamaOk();
      return { response, provider };
    } catch (err) {
      ...
      lastError = err;
      if (i < chain.length - 1) {
        logger.warn(`LLM stream failed for ${provider}: ${err.message} — trying next provider`);
      }
    }
  }

  throw new Error(lastError ? `All LLM streaming providers failed. Last error: ${lastError.message}` : "All LLM providers unavailable");
}
