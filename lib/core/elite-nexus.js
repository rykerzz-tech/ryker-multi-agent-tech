...
class DynamicAgentRouter {
  route(input, taskClassifications, complexityLevel = "LOW") {
    ...
    const specialistMap = {
      ...
      testing: { name: "QA_TestSpecialist", fileName: "test-engineer", capabilities: ["Unit Testing", "Jest", "Assertions", "Regression"] },
      ...
      mobile: { name: "MobileSpecialist", fileName: "mobile-specialist", capabilities: ["React Native", "Flutter", "Expo"] },
      ...
      database: { name: "DatabaseSpecialist", fileName: "database-specialist", capabilities: ["Prisma", "Drizzle", "SQL Migrations"] },
    ...
    const primarySpecialist = specialists.length > 0 ? specialists[0].name : "BackendSpecialist";
    const capabilities = specialists.length > 0 ? specialists[0].capabilities : ["General Engineering"];
    const backupSpecialists = specialists.slice(1).map(r => r.name);
    ...
    if (complexityLevel === "LOW") {
      workflowOrder = ["orchestrator", specialists.length > 0 ? specialists[0].fileName : "backend-specialist", "test-engineer"];
    } else if (complexityLevel === "MEDIUM") {
      workflowOrder = ["orchestrator", "explorer-agent", "project-planner"];
      specialists.forEach(s => {
        if (!workflowOrder.includes(s.fileName)) workflowOrder.push(s.fileName);
      });
...
class EliteNexusOS {
  ...
  getActiveAgentDisplay(agentName, input = "", showFullTrace = false) {
    ...
    if (name === "orchestrator") {
      ...
      return `🔍 Explorer กำลังสำรวจโปรเจกต์`;
    } else if (name === "frontend-specialist") {
      return `💻 Frontend Specialist กำลังพัฒนา`;
    } else if (name === "security-specialist") {
      return `🛡️ Security Specialist กำลังตรวจสอบ`;
    } else if (name === "test-engineer") {
      return `🧪 Test Engineer กำลังทดสอบ`;
    } else if (name === "devops-specialist") {
      return `🚀 DevOps Specialist กำลังดำเนินการ`;
    } else if (name === "project-planner") {
      return `📅 Project Planner กำลังวางแผนงาน`;
    } else if (name === "requirements-analyst") {
      return `📋 Requirements Analyst กำลังวิเคราะห์ความต้องการ`;
    } else if (name === "mobile-specialist") {
      return `📱 Mobile Specialist กำลังพัฒนา`;
    } else if (name === "integration-specialist") {
      return `🔗 Integration Specialist กำลังเชื่อมต่อระบบ`;
    } else if (name === "api-specialist") {
      return `🔌 API Specialist กำลังออกแบบ/พัฒนา API`;
    } else if (name === "uiux-specialist") {
      return `🎨 UIUX Specialist กำลังออกแบบประสบการณ์ผู้ใช้`;
    } else if (name === "accessibility-specialist") {
      return `♿ Accessibility Specialist กำลังตรวจสอบการเข้าถึง`;
    } else if (name === "seo-specialist") {
      return `📈 SEO Specialist กำลังปรับปรุง SEO`;
    } else if (name === "content-specialist") {
      return `✍️ Content Specialist กำลังจัดการเนื้อหา`;
    } else if (name === "database-specialist") {
      return `🗄️ Database Specialist กำลังจัดการฐานข้อมูล`;
    } else if (name === "data-engineer") {
      return `📊 Data Engineer กำลังจัดการข้อมูล`;
    } else if (name === "ai-ml-specialist") {
      return `🤖 AI/ML Specialist กำลังพัฒนาโมเดล AI`;
    } else if (name === "prompt-engineer") {
      return `💬 Prompt Engineer กำลังจัดการ prompt`;
    } else if (name === "cloud-architect") {
      return `☁️ Cloud Architect กำลังออกแบบระบบคลาวด์`;
    } else if (name === "sre-specialist") {
      return `🚨 SRE Specialist กำลังตรวจสอบความเสถียร`;
    } else if (name === "network-specialist") {
      return `🌐 Network Specialist กำลังจัดการเครือข่าย`;
    } else if (name === "debugger") {
      return `🐞 Debugger กำลังแก้บั๊ก`;
    } else if (name === "performance-specialist") {
      return `⚡ Performance Specialist กำลังปรับปรุงความเร็ว`;
    } else if (name === "code-reviewer") {
      return `🔎 Code Reviewer กำลังตรวจสอบคุณภาพโค้ด`;
    } else if (name === "documentation-specialist") {
      return `📝 Documentation Specialist กำลังจัดทำเอกสาร`;
    } else if (name === "release-manager") {
      return `📦 Release Manager กำลังเตรียมการเผยแพร่`;
    } else if (name === "migration-specialist") {
      return `🔄 Migration Specialist กำลังย้ายระบบ`;
    } else {
  ...
  generateReport(state, mode = "NORMAL") {
    ...
    const lines = [];
    const categories = state.quality?.categories || classification.categories || [];
    
    // Helper to determine modified areas (Frontend, Backend, Database, etc.)
    const modifiedAreas = [];
    if (categories.includes("frontend")) modifiedAreas.push("Frontend");
    if (categories.includes("backend")) modifiedAreas.push("Backend");
    if (categories.includes("database")) modifiedAreas.push("Database");
    if (modifiedAreas.length === 0) {
      modifiedAreas.push(changedFiles.length > 0 ? changedFiles.map(f => path.basename(f)).join(", ") : "General");
    }
    const modifiedStr = modifiedAreas.join(", ");

    const checksPassed = testStatus === "PASS" || testStatus === "NOT_APPLICABLE";
    const securityPassed = buildStatus === "PASS";
    ...
    if (hasError) {
      ...
      lines.push("");
      lines.push(`ดำเนินการ: ${taskSummary}`);
      lines.push(`ตรวจสอบ: ${verifications}`);
      lines.push(`สถานะ: ${finalStatus}`);
      lines.push("━━━━━━━━━━━━━━━━━━━━━━");
      ...
      lines.push("✅ เสร็จสิ้น");
      lines.push("");
      lines.push(`งาน: ${taskSummary}`);
      lines.push(`ส่วนที่แก้ไข: ${modifiedStr}`);
      lines.push(`การตรวจสอบ: ${verifications}`);
      lines.push(`ปัญหา: ไม่มี`);
      lines.push(`สถานะ: ${finalStatus}`);
      lines.push("━━━━━━━━━━━━━━━━━━━━━━");
    }
