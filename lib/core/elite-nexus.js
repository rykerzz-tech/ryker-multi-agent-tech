...
const { getComplexity, evaluateLayers, getVerificationStatus, SelfReflectionEngine } = require("./quality-gate");
...
class CognitiveDecomposer {
  ...
  decompose(input, context = {}) {
    ...
    const isCode = /(code|build|refactor|feature|bug|fix|api|ui|test|endpoint|database)/i.test(raw);
    ...
    if (isCode) {
      tasks.push({
        taskId: "task-0",
        label: "Architecture & Context Discovery",
        description: "Analyze project context, verify codebase dependencies, and formulate plan.",
        dependsOn: [],
        parallelizable: false,
        estimatedComplexity: 3,
        assignedAgentRole: "Architect",
        retryPolicy: { maxRetries: 3, backoffMs: 1000, escalateOnFail: true },
        status: "PENDING",
      });

      tasks.push({
        taskId: "task-1",
        label: "Core Implementation & Logic",
        description: "Implement code changes with surgical precision and clean architecture.",
        dependsOn: ["task-0"],
        parallelizable: false,
        estimatedComplexity: 6,
        assignedAgentRole: "BackendSpecialist",
        retryPolicy: { maxRetries: 3, backoffMs: 1500, escalateOnFail: true },
        status: "PENDING",
      });

      tasks.push({
        taskId: "task-2",
        label: "UI & Interface Integration",
        description: "Build user-facing controls, components, or client interfaces.",
        dependsOn: ["task-0"],
        parallelizable: true,
        estimatedComplexity: 5,
        assignedAgentRole: "FrontendSpecialist",
        retryPolicy: { maxRetries: 3, backoffMs: 1000, escalateOnFail: true },
        status: "PENDING",
      });
      ...
      tasks.push({
        taskId: "task-3",
        label: "Verification & Quality Gate",
        description: "Execute test suite, check linting, and verify quality gate constraints.",
        dependsOn: ["task-1", "task-2"],
        parallelizable: false,
        estimatedComplexity: 4,
        assignedAgentRole: "TestEngineer",
        retryPolicy: { maxRetries: 2, backoffMs: 1000, escalateOnFail: true },
        status: "PENDING",
      });
    } else {
      tasks.push({
        taskId: "task-0",
        label: "Context Retrieval & Extraction",
        description: "Query persistent memory and parse user objectives.",
        dependsOn: [],
        parallelizable: false,
        estimatedComplexity: 2,
        assignedAgentRole: "Explorer",
        retryPolicy: { maxRetries: 3, backoffMs: 1000, escalateOnFail: true },
        status: "PENDING",
      });
      ...
      tasks.push({
        ...
        description: "Synthesize structured answer and evaluate quality scores.",
        dependsOn: ["task-0"],
        parallelizable: false,
        estimatedComplexity: 4,
        assignedAgentRole: "GeneralistSpecialist",
        retryPolicy: { maxRetries: 3, backoffMs: 1000, escalateOnFail: true },
        status: "PENDING",
      });
    }

    const subTasks = tasks.map((t, idx) => {
      let phase = "ANALYSIS";
      if (idx === 0) phase = "ANALYSIS";
      else if (idx === tasks.length - 1) phase = "VERIFICATION";
      else if (t.label.toLowerCase().includes("database") || t.label.toLowerCase().includes("data") || t.description.toLowerCase().includes("data") || idx === 1) phase = "DATA_LAYER";
      ...
      return {
        ...
        phase,
        title: t.description,
        owner: t.assignedAgentRole,
        inputs: t.dependsOn,
        outputCriteria: "Satisfies quality gate constraints",
        dependencies: t.dependsOn
      };
    });

    return {
      dagId: `dag-${crypto.randomBytes(4).toString("hex")}`,
      taskId: `plan-${crypto.randomBytes(4).toString("hex")}`,
      taskCount: tasks.length,
      isDecomposed: tasks.length > 1,
      input: raw,
      tasks,
      subTasks,
      estimatedComplexity: tasks.length > 3 ? "HIGH" : "MEDIUM",
      createdAt: Date.now(),
    };
  }
}
...
class DAGExecutor {
  ...
  async execute(dag, taskRunner, onProgress = () => {}) {
    ...
    const isReady = (t) => {
      if (t.status === "DONE" || t.status === "RUNNING" || t.status === "FAILED") return false;
    ...
    while (completedTasks.size + failedTasks.size < tasks.length) {
      ...
      if (readyTasks.length === 0) {
        const remaining = tasks.filter(t => t.status !== "DONE" && t.status !== "FAILED");
        for (const r of remaining) {
          r.status = "FAILED";
          r.error = "Unresolved dependency in DAG";
          failedTasks.add(r.taskId);
        }
        break;
      }
      ...
      await Promise.all(
        readyTasks.map(async (task) => {
          task.status = "RUNNING";
          onProgress({ type: "node:start", node: task, dag: tasks });

          let attempt = 0;
          let success = false;
          let lastErr = null;
          const maxRetries = task.retryPolicy?.maxRetries || 1;

          while (attempt < maxRetries && !success) {
            attempt++;
            try {
              const res = await taskRunner(task, results);
              results[task.taskId] = res;
              task.status = "DONE";
              task.result = res;
              completedTasks.add(task.taskId);
              success = true;
              onProgress({ type: "node:done", node: task, result: res, dag: tasks });
            } catch (err) {
              lastErr = err;
              if (attempt < maxRetries) {
                await new Promise(r => setTimeout(r, task.retryPolicy?.backoffMs || 500));
              }
            }
          }
          ...
          if (!success) {
            task.status = "FAILED";
            task.error = lastErr?.message || "Task failed";
            failedTasks.add(task.taskId);
            onProgress({ type: "node:fail", node: task, error: task.error, dag: tasks });
          }
        })
      );
    }

    onProgress({ type: "dag:complete", dag: tasks, results });
    return {
      dagId: dag.dagId,
      tasks,
      results,
...
class SwarmOrchestrator {
  constructor(decomposer, router, reflectionEngine) {
    this.decomposer = decomposer || new CognitiveDecomposer();
    this.router = router;
    this.reflectionEngine = reflectionEngine || new SelfReflectionEngine();
  }

  createSwarmPlan(input, context = {}) {
    const plan = this.decomposer.decompose(input, context);
    const swarmTeam = [
      { role: "Architect", agent: "orchestrator", phase: "Strategy & Boundary Control" },
      { role: "Specialist", agent: "primary-specialist", phase: "Surgical Implementation" },
      { role: "Reviewer", agent: "code-reviewer", phase: "Quality & Security Inspection" },
      { role: "Verifier", agent: "test-engineer", phase: "Self-Reflection & Runtime Verification" }
    ];
  ...
  async fanOut(task, agentRoles, runner) {
    const runs = await Promise.all(
      agentRoles.map(async (role) => {
        try {
          const res = await runner(role, task);
          const reflection = SelfReflectionEngine.reflect(res?.content || res, task);
          return { role, result: res, score: reflection.aggregateScore, reflection };
        } catch (err) {
          return { role, error: err.message, score: 0 };
        }
      })
    );
  ...
  async pipeline(task, agentPipeline, runner) {
    let currentInput = task;
    const history = [];

    for (const role of agentPipeline) {
      const stepRes = await runner(role, currentInput);
      const content = stepRes?.content || String(stepRes || "");
      history.push({ role, input: currentInput, output: content });
      currentInput = content;
    }

    return {
      finalOutput: currentInput,
      pipelineSteps: history,
    };
  }

  async voting(task, agentRoles, runner) {
    const results = await Promise.all(
      agentRoles.map(async (role) => {
        const res = await runner(role, task);
        const text = typeof res === "string" ? res : res?.content || JSON.stringify(res);
        const reflection = SelfReflectionEngine.reflect(text, task);
        return { role, text, score: reflection.aggregateScore };
      })
    );
  ...
  handoff(fromAgent, toAgent, context = {}) {
    const handoffRecord = {
      handoffId: `hoff-${crypto.randomBytes(4).toString("hex")}`,
      fromAgent,
      toAgent,
      timestamp: Date.now(),
      context: {
    ...
    logger.info(`[SwarmHandoff] Protocol handoff: ${fromAgent} ➔ ${toAgent}`);
...
class EliteNexusOS {
  constructor() {
    ...
    this.cognitive = {
      decomposer: new CognitiveDecomposer(),
      reflectionEngine: new SelfReflectionEngine(),
      swarm: new SwarmOrchestrator(new CognitiveDecomposer(), this.router.agentRouter, new SelfReflectionEngine())
    };
    this.reflection = {
