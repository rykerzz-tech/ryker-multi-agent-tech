...
const ALLOWED_COMMANDS = [
  "python3", "python", "py", "node", "git", "npm", "npx", "bun",
  ...
  _tmpCleanupTimer = setInterval(() => {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.startsWith("ryker-multi-agent-tech-")) {
...
function safeWrite(filePath, content, encoding = "utf-8", projectRoot) {
  ...
  const tmp = path.join(os.tmpdir(), `ryker-multi-agent-tech-${crypto.randomUUID()}`);
  try {
...
function sandboxExec(cmd, args = [], options = {}) {
  ...
  if (process.platform === "win32") {
    const localAppData = process.env.LOCALAPPDATA || (process.env.USERPROFILE ? path.join(process.env.USERPROFILE, "AppData", "Local") : "C:\\Users\\klaka\\AppData\\Local");
    const gitCmdPath = path.join(localAppData, "Programs", "Git", "cmd");
    if (fs.existsSync(gitCmdPath) && (!env.PATH || !env.PATH.includes(gitCmdPath))) {
