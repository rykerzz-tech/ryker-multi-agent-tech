...
function packageAgent(projectDir, options = {}) {
  ...
  if (!cfgDir) {
    throw new Error("No config directory found. Run `ryker-multi-agent-tech init` first.");
  }
  ...
  const tmpDir = path.join(os.tmpdir(), `ryker-multi-agent-tech-publish-${agentName}-${crypto.randomUUID()}`);
  ...
  const agentPkg = {
    ...
    dependencies: {
      "ryker-multi-agent-tech": "^2.0.0",
    },
    keywords: ["ryker-multi-agent-tech", "ai-agent", "agent", agentName],
...
console.log("Use: ryker-multi-agent-tech run \"your task here\"");
`;
...
## Use with Ryker MultiAgent Tech

\`\`\`bash
npx ${pkgName}
npx ryker-multi-agent-tech run "your task here"
\`\`\`
