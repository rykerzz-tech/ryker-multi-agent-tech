...
// API Key authentication — enabled when RYKER_API_KEY env var is set
const API_KEY = process.env.RYKER_API_KEY || null;
...
function wsApiKeyAuth(info, cb) {
  ...
  try {
    ...
    const protocolToken = protocolHeader
      .split(",")
      .map((p) => p.trim())
      .find((p) => p.startsWith("ryker-token."));
    const tokenFromProtocol = protocolToken ? protocolToken.slice("ryker-token.".length) : null;
