...
function sensitiveRouteAuth(req, res, next) {
  const API_KEY = process.env.RYKER_API_KEY;
  ...
  if (!isLocal) {
    return res.status(403).json({ error: { code: "FORBIDDEN", message: "Sensitive route requires API key or localhost access. Set RYKER_API_KEY env var." } });
  }
  next();
}
...
function createApp() {
  ...
  if (process.env.RYKER_TRUST_PROXY === "true") {
  ...
  // Global middleware
  const corsOrigin = process.env.RYKER_CORS_ORIGIN || undefined;
  ...
  app.get("/metrics", sensitiveRouteAuth, (req, res) => {
    ...
    // HTTP-specific metrics
    output += `# HELP ryker_http_requests_total Total HTTP requests\n`;
    output += `# TYPE ryker_http_requests_total counter\n`;
    output += `ryker_http_requests_total ${httpRequestsTotal}\n\n`;
    ...
    if (httpRequestDurationMs.length > 0) {
      ...
      output += `# HELP ryker_http_request_duration_seconds HTTP request duration\n`;
      output += `# TYPE ryker_http_request_duration_seconds summary\n`;
      output += `ryker_http_request_duration_seconds{quantile="0.5"} ${(p50 / 1000).toFixed(3)}\n`;
      output += `ryker_http_request_duration_seconds{quantile="0.95"} ${(p95 / 1000).toFixed(3)}\n`;
      output += `ryker_http_request_duration_seconds{quantile="0.99"} ${(p99 / 1000).toFixed(3)}\n`;
      output += `ryker_http_request_duration_seconds_sum ${(sorted.reduce((a, b) => a + b, 0) / 1000).toFixed(3)}\n`;
      output += `ryker_http_request_duration_seconds_count ${sorted.length}\n\n`;
    }
    ...
    try {
      ...
      output += `# HELP ryker_queue_size Current queue size\n`;
      output += `# TYPE ryker_queue_size gauge\n`;
      output += `ryker_queue_size{type="running"} ${qm.running}\n`;
      output += `ryker_queue_size{type="queued"} ${qm.queued}\n\n`;
