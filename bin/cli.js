...
/**
 * Ryker MultiAgent Tech — AI Agent Platform
...
program
  ...
  .addHelpText("before", `\n  ${chalk.cyan(`Ryker MultiAgent Tech v${inline.CURRENT_VERSION}`)} — ${inline.getComponentCounts().agents} Agents | ${inline.getComponentCounts().skills} Skills | ${inline.getComponentCounts().workflows} Workflows\n`)
  .addHelpText("after", `\n  Documentation: https://github.com/rykerzz-tech/ryker-multi-agent-tech#readme\n`);
...
program
  ...
  .action((type, subtype) => {
    console.log(chalk.yellow(`\n  ⚠️ "ryker-multi-agent-tech generate ${type} ${subtype || ""}" is experimental and not yet implemented\n`));
  });
